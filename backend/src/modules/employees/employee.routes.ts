...
employeeRoutes.get("/", async (req, res, next) => {
  try {
    ...
    const where: any = {};
    if (search) where[Op.or] = [{ employeeCode: { [Op.iLike]: `%${search}%` } }, { name: { [Op.iLike]: `%${search}%` } }, { email: { [Op.iLike]: `%${search}%` } }];
    if (req.query.status) where.status = req.query.status;
    if (req.query.departmentId) where.departmentId = req.query.departmentId;
    const include: any[] = [Department, Location, { model: Company, as: "companies", through: { attributes: [] } }];
    if (req.query.companyId) include[2].where = { id: req.query.companyId };
...
employeeRoutes.post("/", async (req, res, next) => {
  try {
    ...
    if (companyIds?.length) await (employee as any).setCompanies(companyIds);
...
employeeRoutes.patch("/:id", async (req, res, next) => {
  try {
    ...
    if (Array.isArray(companyIds)) await (employee as any).setCompanies(companyIds);
