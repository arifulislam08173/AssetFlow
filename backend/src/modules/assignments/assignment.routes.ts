...
assignmentRoutes.get("/", async (req, res, next) => {
  try {
    ...
    if (req.query.companyId) include[0].where = { companyId: req.query.companyId };
...
assignmentRoutes.post("/", async (req, res, next) => {
  try {
    ...
    for (const assetId of body.assetIds) {
      ...
      if (body.companyId && asset.companyId !== body.companyId) throw new ApiError(400, `${asset.assetCode} does not belong to selected company`);
      if (asset.status === "Assigned") throw new ApiError(409, `${asset.assetCode} is already assigned`);
