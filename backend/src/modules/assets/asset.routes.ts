...
assetRoutes.post("/import", async (req, res, next) => {
  try {
    ...
    if (missingHeaders.length) throw new ApiError(400, `CSV template is missing required headers: ${missingHeaders.join(", ")}`);
    ...
    for (let index = 0; index < rows.length; index += 1) {
      ...
      for (const header of TEMPLATE_HEADERS.filter((h) => h !== "notes")) {
        if (!row[header]) addError(header, `${header} is required`);
      }
      const purchasePrice = Number(row.purchasePrice);
      const usefulLifeYears = Number(row.usefulLifeYears);
      if (Number.isNaN(purchasePrice) || purchasePrice < 0) addError("purchasePrice", "Invalid purchase price");
      if (!Number.isInteger(usefulLifeYears) || usefulLifeYears <= 0) addError("usefulLifeYears", "Useful life years must be a positive whole number");
      if (Number.isNaN(Date.parse(row.purchaseDate))) addError("purchaseDate", "Invalid purchase date");
      if (Number.isNaN(Date.parse(row.warrantyExpiry))) addError("warrantyExpiry", "Invalid warranty expiry date");
      if (seenAssetCodes.has(row.assetCode)) addError("assetCode", "Duplicate asset code inside CSV");
      if (seenSerialNumbers.has(row.serialNumber)) addError("serialNumber", "Duplicate serial number inside CSV");
      ...
      if (!company) addError("companyCode", "Company not found by code");
      if (!category) addError("categoryName", "Category not found by name");

      let vendor: Vendor | null = null;
      let location: Location | null = null;
      if (company && row.vendorName) vendor = await findOrCreateVendorByName(row.vendorName, company.id);
      if (row.locationName) location = await findOrCreateLocationByName(row.locationName);

      const duplicate = await Asset.findOne({ where: { [Op.or]: [{ assetCode: row.assetCode }, { serialNumber: row.serialNumber }] } });
      if (duplicate?.assetCode === row.assetCode) addError("assetCode", "Asset code already exists");
      if (duplicate?.serialNumber === row.serialNumber) addError("serialNumber", "Serial number already exists");

      const rowHasErrors = errors.some((e) => e.row === rowNumber);
      if (rowHasErrors || !company || !category || !vendor || !location) continue;
